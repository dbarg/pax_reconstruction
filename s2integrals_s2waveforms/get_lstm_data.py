...
def get_data(df_events, s2_window_max):
    ...
    train_data  = np.zeros((nEvents*n_channels, s2_window_max))
    train_truth = np.zeros((nEvents*n_channels, 1))
    ...
    for iEvent, event_num in enumerate(lst_events):
        ...
        for iChannel, row in df_s2waveforms.iterrows():
            ...
            #event_train_data[iIndex, :] = arr_channel_padded # timeseries for iChannel
            train_data[iIndex, :]  = arr_channel_padded # timeseries for iChannel
            train_truth[iIndex, :] = channel_integral
